...
class BaseTracker(ABC):
    ...
    def id_to_color(self, id: int, saturation: float = 0.75, value: float = 0.95) -> tuple:
        """
        Generates a consistent unique BGR color for a given ID using a predefined range of hues.
        The colors will cycle through a set of distinct colors like red, blue, orange, green, etc.
        ...
        # List of predefined hues corresponding to distinct colors
        predefined_hues = [0, 15, 30, 45, 60, 75, 120, 140, 160, 180, 210,240,255, 270, 300,310,330]  # Red, orange, yellow, green, cyan, blue, purple, pink
        ...
        # Convert the first few characters of the hash to an integer
        # Use the hash to cycle through the predefined hues
        hue_index = int(hash_digest[:8], 16) % len(predefined_hues)
        hue = predefined_hues[hue_index] / 360.0  # Convert to a fraction between 0 and 1 for HSV
    ...
    def plot_box_on_img(self, img: np.ndarray, box: tuple, conf: float, cls: int, id: int, thickness: int = 4, fontscale: float = 0.5) -> np.ndarray:
        """
    ...
    def plot_results(self, img: np.ndarray, show_trajectories: bool, thickness: int = 4, fontscale: float = 0.5) -> np.ndarray:
        """
